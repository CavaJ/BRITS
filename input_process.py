...
out = pd.read_csv('./raw/Outcomes-ab.txt').set_index('RecordID')['In-hospital_death']
...
def parse_data(x):
    ...
    for attr in attributes:
        if attr in x:  # python 3.7  # if x.has_key(attr):
...
def parse_rec(values, masks, evals, eval_masks, dir_):
    ...
    # only used in GRU-D
    forwards = pd.DataFrame(values).fillna(method='ffill').fillna(
        0.0).as_matrix()
...
def parse_id(id_):
    ...
    # TODO obtain record id from here and put it to the rec
