...
def train(model):
    ...
    # now generate average auroc and auprc over epochs
    sum_auroc = 0.0
    sum_auprc = 0.0
    max_auroc = 0.0
    max_auprc = 0.0
    for tuple in auroc_auprc:
        auroc = tuple[0]
        auprc = tuple[1]
        sum_auroc = sum_auroc + auroc
        sum_auprc = sum_auprc + auprc
        if auroc > max_auroc:
            max_auroc = auroc
        if auprc > max_auprc:
            max_auprc = auprc

    print('Max AUROC {}'.format(max_auroc), 'Max AUPRC {}'.format(max_auprc))
    print('Avg. AUROC {}'.format(sum_auroc / len(auroc_auprc)), 'Avg. AUPRC {}'.format(sum_auprc / len(auroc_auprc)))
